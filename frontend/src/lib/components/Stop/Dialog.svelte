...
<Dialog.Content name={'stop/' + stop.id} let:title let:description let:close>
	...
	<div use:melt={description}>
		<div
			use:melt={$root}
			class="flex border mb-12 min-w-80 border-neutral-800 flex-col rounded-xl shadow-lg data-[orientation=vertical]:flex-row bg-neutral-900/50 text-indigo-400"
		>
			<div
				use:melt={$list}
