...
pub async fn import(pool: &PgPool) -> Result<(), ImportError> {
    ...
    for entity in feed.entity {
        ...
        alerts.push(alert);
        active_periods.extend(alert_active_periods);
        affected_entities.extend(alert_affected_entities);
    }
...
impl Alert {
    ...
    pub async fn insert(
        ...
        let ids = values.iter().map(|a| a.id).collect::<Vec<Uuid>>();
