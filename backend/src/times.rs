    ...
    tokio::spawn(async move {
        loop {
            for endpoint in ENDPOINTS.iter() {
                ...
                decode_feed(tx_clone, &endpoint).await;
            }
    ...
    tokio::spawn(async move {
        ...
        while let Some(entity) = rx.recv().await {
            if let Some(trip_update) = entity.trip_update {
                ...
                let route_id = match trip_update.trip.route_id.as_ref() {
                    Some(id) => id,
                    None => continue,
                };
...
pub async fn decode_feed(tx: Sender<FeedEntity>, endpoint: &str) {
