...
impl PchipInterpolator {
    ...
    ///
    /// # Errors
    /// Returns an error if `x.len() != y.len()` or `x.len() < 2`.
    pub fn try_new(x: &[f64], y: &[f64]) -> anyhow::Result<Self> {
        if x.len() != y.len() {
            anyhow::bail!("x and y must have the same length");
        }
        let n = x.len();
        if n < 2 {
        ...
        for i in 0..n - 1 {
            ...
            if hi <= 0.0 {
                anyhow::bail!(
                    "x values must be strictly increasing: x[{i}]={} >= x[{}]={}",
                    x[i],
                    i + 1,
                    x[i + 1]
                );
    ...
    /// Evaluate the derivative of the interpolant at a single point.
    ///
    /// Returns `None` if `x_eval` is outside the interpolation domain.
    pub fn evaluate_derivative(&self, x_eval: f64) -> Option<f64> {
        ...
        if (x_eval - self.x[0]).abs() < 1e-12 {
            ...
            return Some(self.c1[0] / h);
        }
        if (x_eval - self.x[n - 1]).abs() < 1e-12 {
            let seg = n - 2;
            let h = self.x[seg + 1] - self.x[seg];
            return Some((self.c1[seg] + 2.0 * self.c2[seg] + 3.0 * self.c3[seg]) / h);
        }
        ...
        Some((self.c1[seg] + 2.0 * self.c2[seg] * s + 3.0 * self.c3[seg] * s * s) / h)
    }
}
